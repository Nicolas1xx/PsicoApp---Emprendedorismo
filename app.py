...
# ROTA DE EDIÇÃO (Corrigida e Limpa de Caracteres Inválidos)
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '' and allowed_file(file.filename):
            ...
            try:
                ...
                # No Render, isso vai falhar, mas o 'except' vai capturar.
                file.save(os.path.join(app.config['UPLOAD_FOLDER'], unique_filename))
                avatar_filename = unique_filename
                flash("Nova foto de perfil enviada com sucesso.", 'info')
            except Exception as e:
                # Adiciona log para o Render
                app.logger.warning(f"Falha ao salvar upload local no Render: {e}") 
                flash(f"Aviso: Erro ao salvar a nova foto (upload local não permitido no Render): {e}. A foto antiga será mantida.", 'warning')
        ...
        try:
            ...
            # Adiciona log para o Render
            app.logger.error(f"Erro ao atualizar psicologo no Firestore: {e}") 
        ...
        if db:
            try:
                ...
                # Adiciona log para o Render
                app.logger.error(f"Erro ao buscar psicologos no GET de editar_psicologo: {e}") 
                pass 
